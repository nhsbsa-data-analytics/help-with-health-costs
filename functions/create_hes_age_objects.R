...
#'
create_hes_age_objects <- function(db_connection, db_table_name, service_area, min_ym, max_ym, focus_fy, subtype_split = FALSE){
  ...
  if(subtype_split == TRUE){
    ...
    chart_fields <- c('SERVICE_AREA_NAME', 'ISSUE_FY', 'CUSTOM_AGE_BAND') # subtype not required

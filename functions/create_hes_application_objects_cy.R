#' create_hes_application_objects_cy
#'
#' Create objects to summarise the HES application data
#' Output is data for supplementary datasets
#' TO DO: rewrite main create_hes_application_objects function to allow financial year or calendar year
#' 
...
#'
create_hes_application_objects_cy <- function(db_connection, db_table_name, service_area, min_ym, max_ym, subtype_split = FALSE){
  ...
  if(subtype_split == TRUE){
    ...
    if(service_area %in% c("MAT","MED","PPC","HRTPPC")){
      obj_suppData <- get_hes_application_data(con, db_table_name, service_area, min_ym, max_ym, c('SERVICE_AREA_NAME', 'CERTIFICATE_SUBTYPE', 'APPLICATION_CY')) |> 
        dplyr::mutate(COUNTRY = 'n/a') |> 
        dplyr::relocate(COUNTRY, .after = APPLICATION_CY) |> 
        dplyr::arrange(CERTIFICATE_SUBTYPE, APPLICATION_CY) |> 
        rename_df_fields()
      
    } else {
      obj_suppData <- get_hes_application_data(con, db_table_name, service_area, min_ym, max_ym, c('SERVICE_AREA_NAME', 'CERTIFICATE_SUBTYPE', 'COUNTRY', 'APPLICATION_CY')) |> 
        dplyr::arrange(CERTIFICATE_SUBTYPE, COUNTRY, APPLICATION_CY) |> 
        rename_df_fields()
    }
    ...
    # for "England only" services use a n/a placeholder for country
    if(service_area %in% c("MAT","MED","PPC","HRTPPC")){
      obj_suppData <- get_hes_application_data(con, db_table_name, service_area, min_ym, max_ym, c('SERVICE_AREA_NAME', 'APPLICATION_CY')) |> 
        dplyr::mutate(COUNTRY = 'n/a') |> 
        dplyr::relocate(COUNTRY, .after = APPLICATION_CY) |> 
        dplyr::arrange(APPLICATION_CY) |> 
        rename_df_fields()
    } else {
      obj_suppData <- get_hes_application_data(con, db_table_name, service_area, min_ym, max_ym, c('SERVICE_AREA_NAME', 'COUNTRY', 'APPLICATION_CY')) |> 
        dplyr::arrange(COUNTRY, APPLICATION_CY) |> 
        rename_df_fields()
    }
  }

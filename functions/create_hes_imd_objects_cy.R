#' create_hes_imd_objects_cy
#'
#' Create objects to summarise the HES data by IMD profile
#' Output is data for supplementary datasets
#' TO DO: rewrite main create_hes_imd_objects function to allow financial year or calendar year
#' 
...
#'
create_hes_imd_objects_cy <- function(db_connection, db_table_name, service_area, min_ym, max_ym, subtype_split = FALSE){
  ...
  if(subtype_split == TRUE){
    chart_fields <- c('SERVICE_AREA_NAME', 'CERTIFICATE_SUBTYPE', 'ISSUE_CY', 'IMD_QUINTILE')
    sort_fields_chart <- c('CERTIFICATE_SUBTYPE', 'IMD_QUINTILE')
    tab_align <- "llr"
    if(service_area %in% c("LIS","TAX")){
      supp_fields <- c('SERVICE_AREA_NAME','CERTIFICATE_SUBTYPE', 'COUNTRY', 'ISSUE_CY', 'IMD_QUINTILE')
    } else {
      supp_fields <- chart_fields # country not required
    }
  } else {
    chart_fields <- c('SERVICE_AREA_NAME', 'ISSUE_CY', 'IMD_QUINTILE') # subtype not required
    sort_fields_chart <- c('IMD_QUINTILE')
    tab_align <- "lr"
    if(service_area %in% c("LIS","TAX")){
      supp_fields <- c('SERVICE_AREA_NAME', 'COUNTRY', 'ISSUE_CY', 'IMD_QUINTILE')
    } else {
      supp_fields <- chart_fields # country not required
    }
  }

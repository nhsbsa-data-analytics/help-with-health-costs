#' get_hes_active_data
#'
#' collect active certificate data from the HES dataset
...
#'
get_hes_active_data <- function(db_connection, db_table_name, service_area, min_ym, max_ym, group_list) {
  ...
  # create a list of financial years to use
  # identify a distinct list of financial years
  db_fy_list <- dplyr::tbl(
    con, 
  ...
  # create connection to database table and get details of issued certificates
  db_active <- dplyr::tbl(
    con, 
  ...
  # join datasets to get details, allowing certificates to belong to multiple years
  df_hes_fy_act <- db_fy_list |> 
    dplyr::left_join(
      db_active,
      dplyr::join_by("MAX_YM" >= "CERTIFICATE_START_YM", "MIN_YM" <= "CERTIFICATE_EXPIRY_YM")
    ) |> 
  # summarise, splitting by supplied fields
    dplyr::group_by(across(all_of(group_list))) |> 
    dplyr::summarise(ACTIVE_CERTS = n(), .groups = "keep") |> 
    dplyr::arrange(FINANCIAL_YEAR) |> 
    dplyr::collect()
  
  # return output
  return(df_hes_fy_act)

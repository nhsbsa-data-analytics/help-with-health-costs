#' get_hes_application_data
#'
#' collect application data from the HES dataset
...
#'
get_hes_application_data <- function(db_connection, db_table_name, service_area, min_ym, max_ym, group_list) {
...
# Test Parameters ---------------------------------------------------------
  ...
  if(DBI::dbExistsTable(conn = db_connection, name = DBI::Id(schema = toupper(db_connection@info$username), table = db_table_name)) == FALSE){
    stop(paste0("Invalid parameter (db_table_name) supplied to get_hes_application_data: ", db_table_name, " does not exist!"), call. = FALSE)
  }
  ...
  if(is.na(as.Date(paste0(substr(min_ym,1,4),'-',substr(min_ym,5,6),'-01'), optional = TRUE) == TRUE)){
    stop("Invalid parameter (min_ym) supplied to get_hes_application_data. Must be valid year_month (YYYYMM)", call. = FALSE)
  }
  # abort if too early (pre April 2015) or later than current month
  if(min_ym < 201504 | min_ym > format(Sys.Date(),'%Y%m')){
    stop("Invalid parameter (min_ym) supplied to get_hes_application_data. Must be valid year_month (YYYYMM) between 201504 and current month", call. = FALSE)
  }
  
  # Test Parameter: max_ym
  # abort if not a valid YM
  if(is.na(as.Date(paste0(substr(max_ym,1,4),'-',substr(max_ym,5,6),'-01'), optional = TRUE) == TRUE)){
    stop("Invalid parameter (max_ym) supplied to get_hes_application_data. Must be valid year_month (YYYYMM)", call. = FALSE)
  }
  # abort if too early (pre April 2015) or later than current month
  if(max_ym < 201504 | max_ym > format(Sys.Date(),'%Y%m')){
    stop("Invalid parameter (max_ym) supplied to get_hes_application_data. Must be valid year_month (YYYYMM) between 201504 and current month", call. = FALSE)
  }

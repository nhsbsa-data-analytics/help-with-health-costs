...
These statistics use the [National Statistics Postcode Lookup (NSPL) - May 2023](https://geoportal.statistics.gov.uk/datasets/379c875939ec4fc4bb48ed32711eb042/about) to identify ICB and country for each individual. This uses the captured postcode information supplied for the application/certificate.
...
Some help with health cost services are generally limited to applicants from England whilst other services may also provide certificates for Scotland, Wales and Northern Ireland. Within the main report, figures will represent all activity. Where applicable the supporting dataset will split the country by either England, Other or Unknown. 
...
For some help with health cost services, such a NHS Low Income Scheme, a single application could represent multiple people. However, the figures in this report are based on a certificate level and therefore figures will represent a single application or certificate, regardless of how many individuals were included on the application.
...
For some help with health cost services, such a NHS Low Income Scheme, a single certificate could represent multiple people. However, the figures in this report are based on a certificate level and therefore figures will represent a single application or certificate, regardless of how many individuals were included on the application.

#### 2.2.3 Active certificate counts
Where a count of active certificates has been provided this will only include cases where a certificate has been issued.
...
For "active" certificates data can only be reported from 2019/20 onwards. This is due to historic data retention schedules and the potential length of issued certificates.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For NHS Low Income Scheme data this includes where the calculated age would be less than 15 or greater than 99.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For maternity exemption data this includes where the calculated age would be less than 15 or greater than 60.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For medical exemption data this includes where the calculated age would be less than 15 or greater than 60.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For PPC data this includes where the calculated age would be less than 15 or greater than 60.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For HRT PPC data this includes where the calculated age would be less than 15 or greater than 60.
...
To limit risk of inaccurate reporting, a classification of "Not Available" represents data with confidence issues. For NHS Low Income Scheme data this includes where the calculated age would be less than 15 or greater than 90.
...
Some help with health cost services are focused on providing support with NHS prescription costs. For these services the most appropriate base population may be estimated patient counts based on data captured from NHS prescribing data. 
...
As of June 2024 the 12 month rolling accuracy results for help with health cost service areas were:
...
Age, and therefore age bands, have been calculated based on the applicant's date of birth and the date the application was received. Where the calculated age would suggest potential data quality issues, data has been reported as "Not Available".
...
The number of active certificates is based solely on the start and expiration date of the issued certificates. This does not consider if the certificate holder was using the certificate or not. Certificates will be included if they were valid for one or more days during the reported financial year.

Some individuals may have multiple certificates active in a single financial year, with each certificate being counted as an distinct event.

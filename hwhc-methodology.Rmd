...
Some help with health cost services are ring-fenced to applicants from England whilst other services may also provide certificates for Scotland, Wales and Northern Ireland. Within the main report, figures will represent all activity. The supporting datasets will include a country breakdown. 
...
As deprivation is based on indices calculated for England only, reporting is limiting to applicants that can be identified with an English postcode.
...
A single applicant could submit multiple applications during the same periods. Each application will be treated as a unique event and counted accordingly.

For some help with health cost services, such a NHS Low Income Scheme, a single application could represent multiple people.

#### 2.2.2 Issued certificate counts
Where a count of issued certificates has been provided this will only include cases where the application has been completed and a certificate (or outcome for NHS Low Income Scheme) issued to the applicant.

These counts are based solely on certificates issued by NHS BSA. There is no ability to identify if the certificate was received and then used by the applicant.
...
For NHS Low Income Scheme, applications are handled via an assessment process and may not qualify for support. Applicants will be informed of the outcome of the assessment, with either a certificate being issued or confirmation of no support. Outcome decisions of no support will be reported alongside the HC2 and HC3 certificates issued. Where no certificate is issued, the time period will represent the date of the application as no date is available for when the no support decision was issued to the applicant.

#### 2.2.3 Active certificate counts
Where a count of active certificates has been provided this will only include cases where a certificate has been issued to the customer.

A certificate will be deemed "active" if the start and expiration date of the certificate cover one or more days in the reported time period.

There is no ability to identify if a certificate is actively being used by the certificate holder.
...
##### 2.3.2.3 Proportion of certificates issued relative to expected due date
This measure shows the relationship between the date the certificate was issued to the applicant and the expected due date. Proportion figures are cumulative and therefore by 12 months after the due date all certificates must have been issued.

The expected due date will typically be supplied when the application is submitted and therefore may not always represent the actual date of delivery.

Figures have been reported by number of months, with figures rounded to the nearest number of months. For example, using an expected due date of 10 January 2024:

* certificates issued on 26 June 2023 would show as -7 months
* certificates issued on 25 December 2023 would show as -1 months
* certificates issued on 26 December 2023 would show as 0 months
* certificates issued on 25 January 2024 would show as 0 months
* certificates issued on 26 January 2024 would show as 1 months
* certificates issued on 30 March 2024 would show as 3 months
* if no expected due date is available, or the captured due date appears invalid, figures will be reported as N/A
...
##### 2.3.3.2 Age
The age reported will represent the age of the applicant at the time of application. Figures are reported in 5 year age bands. The medical exemption certificate provides support with NHS prescriptions charges and therefore is only relevant between the ages of 16 and 59 when prescription items are not automatically free based on patient age.

To limit risk of inaccurate reporting, a classification of "unknown" represents data with confidence issues. For medical exemption data this includes where the calculated age would be less than 15 or greater than 60.
...
##### 2.3.4.2 Age
The age reported will represent the age of the applicant at the time of application. Figures are reported in 5 year age bands. The PPC provides support with NHS prescriptions charges and therefore is only relevant between the ages of 16 and 59 when prescription items are not automatically free based on patient age.

To limit risk of inaccurate reporting, a classification of "unknown" represents data with confidence issues. For PPC data this includes where the calculated age would be less than 15 or greater than 60.
...
##### 2.3.5.2 Age
The age reported will represent the age of the applicant at the time of application. Figures are reported in 5 year age bands. The HRT PPC provides support with NHS prescriptions charges and therefore is only relevant between the ages of 16 and 59 when prescription items are not automatically free based on patient age.

To limit risk of inaccurate reporting, a classification of "unknown" represents data with confidence issues. For HRT PPC data this includes where the calculated age would be less than 15 or greater than 60.
...
To limit risk of inaccurate reporting, a classification of "unknown" represents data with confidence issues. For NHS Low Income Scheme data this includes where the calculated age would be less than 15 or greater than 90.
...
This is the first time this publication has been released.

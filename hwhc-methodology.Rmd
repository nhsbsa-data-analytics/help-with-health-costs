...
Due to the method used to calculate age using the applicants date of birth, some data may change between releases. Historic age data may change:

* if a person updates their data to correct inaccuracies with their date of birth
...
##### 2.3.2.3 Proportion of certificates issued based on certificate duration
...
This measure shows the relationship between the date the certificate was issued to the applicant and the certificate duration. This will identify the pattern of how early in pregnancy certificates are being issued. Proportion figures are cumulative and therefore by 12 months after the birth of the child all certificates must have been issued.

Duration is calculated as the number of months, rounded to the nearest month, between the certificate start and expiration date. If the calculated duration would suggest potential data issues, figures will be reported as N/A.
